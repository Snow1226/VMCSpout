    ...
    public class Mirror : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            if (target != null && renderCamera != null)
            {
                ...
                var imageSizePointLeft = RectTransformUtility.WorldToScreenPoint(renderCamera, new Vector3(-_mirrorRectWidth / 2, -_mirrorRectHeight / 2, 0));
                RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, imageSizePointLeft, renderCamera, out var localLeft);

                var imageSizePointRight = RectTransformUtility.WorldToScreenPoint(renderCamera, new Vector3(_mirrorRectWidth / 2, _mirrorRectHeight / 2, 0));
                RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, imageSizePointRight, renderCamera, out var localRight);

                rect.sizeDelta = new Vector2(Mathf.Abs(localRight.x - localLeft.x), Mathf.Abs(localRight.y - localLeft.y));
            }
        }
